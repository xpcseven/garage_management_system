...
export type LandingSliderSlide = {
  id: string;
  src: string;
  title: string;
};

const FALLBACK_SLIDES: LandingSliderSlide[] = [
  { id: "fallback", src: "/System/Tourism_Images/all-hadar_02.png", title: "الحضر الأثرية" },
];

type Props = {
  slides?: LandingSliderSlide[];
};

export default function Tourism_Img_Component({ slides: slidesProp }: Props) {
  const tourismCards =
    slidesProp && slidesProp.length > 0 ? slidesProp : FALLBACK_SLIDES;
